...
class fuzzer:
	...
	def get_distribution(self):
		...
		coverage_dict=get_distribution.get_dict(CSV,self.n)
		count=0
		for key in coverage_dict:
			count+=1
			value=coverage_dict[key]
			key=key.replace('try_5/','')
			self.coverage[key]=value
		for key in self.coverage:
			self.file_coverage.append(self.coverage[key])
		...
		print len(self.file_sim)
		print len(self.file_coverage)
