...
class VAE:
	...
	def build(self):
		...
		##append
		W_en=tf.Variable(tf.random_normal([self.n_in,64],stddev=0.001))
		b_en=tf.Variable(tf.zeros([64]))
		
		en_h=tf.nn.relu(tf.matmul(self.x,W_en)+b_en)
		##
		
		#latent layer
		W_mu=tf.Variable(tf.random_normal([64,16],stddev=0.001))
		b_mu=tf.Variable(tf.zeros([16]))
		W_sig=tf.Variable(tf.random_normal([64,16],stddev=0.001))
		b_sig=tf.Variable(tf.zeros([16]))
		...
		#decoder layer
		W_de=tf.Variable(tf.random_normal([16,64],stddev=0.001))
		b_de=tf.Variable(tf.zeros([64]))

		h_de=tf.nn.relu(tf.matmul(z,W_de)+b_de)
		
		W_out=tf.Variable(tf.random_normal([64,self.n_in],stddev=0.001))
		...
		#reconstruction
		self.z_in=tf.placeholder(tf.float32,[None,16])
...
def main():
	...
	with tf.Session() as sess:
		saver.restore(sess,"model/def.ckpt")
		
		for epoch_i in range(100):
			#z1=np.random.normal(loc=-2,scale=0.1,size=8)
			#z2=np.random.normal(loc=2,scale=0.1,size=8)
			#z=np.concatenate((z1,z2),axis=0)
			z=np.random.normal(size=16)
			z=np.array([z]).reshape(1,16)
