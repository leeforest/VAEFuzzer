...
class fuzzer:
	...
	def seed_analyzer(self):			
		...
		command='cp '+self.seed+' hwp_seeds/try_4/m'+str(self.iteration)+'.hwp'
		os.system(command)
	...
	def fuzz(self):
		...
		print '\ncrash count: %d\n' %len(segfault_list)
		f=open('results/try_4_hwp_coverage_softmax_100.csv','a')
		
		#apply softmax
		for i in range(len(Y)):
			Y[i]=float(Y[i])/100000
		Y=self.softmax(Y)
	...
	def start_debugger(self,i,coverage_dict,X,Y,segfault_list):
		...
		test_file='hwp_seeds/try_4/m'+str(self.iteration)+'.hwp'
		#before_file='hwp_seeds/m'+str(self.iteration-1)+'.hwp'
		path_dir="/home/forest/fuzzer/hwp_seeds/try_4/"
		file_list=os.listdir(path_dir)
		rannum=random.randrange(0,len(file_list))
		random_file=file_list[rannum]
		random_file='hwp_seeds/try_4/'+str(random_file)
