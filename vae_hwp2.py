			...
			if epoch_i%50==0:
				...
				for i in range(len(recon[0])):
					...
					if len(tmp)==1:
						tmp='0'+tmp
					hex_string+=tmp
				#hex_string='89504E470D0A1A0A'+hex_string[16:]
				#hex_string=hex_string[0:-16]+'49454E44AE426082'
				#print(hex_string[:30])
				hex_string=binascii.unhexlify(hex_string)
				#print(hex_string[:30])
				#before write, copy original 
				orig='results\\Bin0002\\test.hwp'
				recon_file='results\\Bin0002\\m'+str(epoch_i)+'.hwp'
